...
#   Modes:
#       0 - Aliveness Check
#       1 - Functional (Take 10 photos with default settings)
#       2 - Take 1 picture with editable input values
# ============================================================================


def PayloadUSAFAST(start_time, mode, duration, exposure, gain)
    ...
    begin
       ...
       # turning up 
        ...
        when 1 #taking 10 photos with default settings and sending to GS
            cmd("MAX_FSX MISC_EVR with STRING 'E_INFO: Seq: PayloadUSAFAST, Beginning Mode #{mode}: Take Photos with Default Settings'") 
            wait(1)
        ...
        when 2 #taking 1 picture with edited input values
            cmd("MAX_FSX MISC_EVR with STRING 'E_INFO: Seq: PayloadUSAFAST, Beginning Mode #{mode}: Take Photos with Modified Settings'") 
            wait(1)
          
            # Take pictures
            cmd("MAX_FSX MISC_EVR with STRING 'E_INFO: Seq: PayloadUSAFAST, Running shell script to take pictures'")
            cmd("MAX_FSX MISC_SYSTEM with STRING 'source /home/root/max/seq/sh/USAFA_ST_take_pictures_exposure.sh exposure=#{value} gain=#{value}'")  
            ...
            wait(1)
            cmd("MAX_FSX MISC_EVR with STRING 'E_INFO: Seq: PayloadUSAFAST, Completed Mode #{mode}: Take Photo with Modified Settings'") 
            wait(1)
        end
